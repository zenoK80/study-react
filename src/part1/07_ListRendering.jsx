  ...
  return(
    <div>
      ...
      <>
      {
        ...
        ? (<ul>{products.filter(p => p.price < 20000).map(p => <li key={p.id}>{p.name} — {p.price.toLocaleString()}원</li>)}</ul>)
        : (<p>조건에 맞는 상품이 없습니다.</p>)
      }
      </>
      ...
      <ul>{products.map(p => { return <li key={p.id}>{p.name}</li>; })}</ul>
      </>
      ...
      <h3>D. 다중 조건 필터</h3>
      {
        ...
        ? (<ul>{filteredProducts.map(p => <li key={p.id}>{p.name} — {p.price.toLocaleString()}원</li>)}</ul>)
        : (<p>조건에 맞는 상품이 없습니다.</p>)
      }
      </>
    </div>
